...
function MovieCard({ movie }: { movie: any }) {
  return (
    <div
      className={`relative group w-[250px] h-[375px] overflow-hidden rounded-lg shadow-md cursor-pointer ${roboto.className}`}
    >
      <Link href={`/film/${movie.id}`} className="block w-full h-full">
        ...
        {/* Hover content (on top of the dim background) */}
